...
export async function POST(request: NextRequest) {
  try {
    ...
    // Platform capabilities that our system supports
    ...
    // Call BAML SourceDiscoveryAgent via Python
    const result = await callBAMLAgent('SourceDiscoveryAgent', {
...
async def main():
    try:
        ...
        if sys.argv[2] == 'SourceDiscoveryAgent':
            result = await b.SourceDiscoveryAgent(
                business_context=params['business_context'],
                platform_capabilities=params['platform_capabilities']
            )
        elif sys.argv[2] == 'FeasibilityAnalyzerAgent':
            result = await b.FeasibilityAnalyzerAgent(
                business_context=params['business_context'],
                recommended_sources=params['recommended_sources']
            )
        elif sys.argv[2] == 'SOWGeneratorAgent':
            result = await b.SOWGeneratorAgent(
                business_context=params['business_context'],
                selected_sources=params['selected_sources'],
                feasibility_analysis=params['feasibility_analysis']
            )
        else:
        ...
        if hasattr(result, 'model_dump'):
            ...
            for item in result:
                if hasattr(item, 'model_dump'):
                    result_dict.append(item.model_dump())
                else:
