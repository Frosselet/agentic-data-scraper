...
export async function POST(request: NextRequest) {
  try {
    ...
    const { businessContext, selectedSources, feasibilityAnalysis } = body

    // Validate required fields
    if (!businessContext || !selectedSources || !feasibilityAnalysis) {
      return NextResponse.json(
        { error: 'Business context, selected sources, and feasibility analysis are required' },
        { status: 400 }
      )
    }

    // Call BAML SOWGeneratorAgent via Python
    const result = await callBAMLAgent('SOWGeneratorAgent', {
