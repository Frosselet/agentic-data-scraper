...
class EntityToTrack(BaseModel):
    ...
    importance: str = Field(..., regex="^(critical|high|medium|low)$")
...
class LocationMention(BaseModel):
    ...
    type: str = Field(..., regex="^(country|region|city|facility|route|market)$")
...
class SpatialContext(BaseModel):
    ...
    geographic_scope: str = Field(..., regex="^(local|regional|national|international|global)$")
...
class StakeholderInvolved(BaseModel):
    ...
    decision_authority: str = Field(..., regex="^(high|medium|low|none)$")
...
class DesiredOutcome(BaseModel):
    ...
    priority: str = Field(..., regex="^(must-have|should-have|nice-to-have)$")
...
class MultiScaleConsideration(BaseModel):
    ...
    scale: str = Field(..., regex="^(local|regional|national|global)$")
...
class TimeSeriesOpportunity(BaseModel):
    ...
    pattern_type: str = Field(..., regex="^(trend|seasonality|cycles|anomalies)$")
    reasoning: str = Field(..., min_length=10, max_length=500)
...
class SemanticStatementOfWork(BaseModel):
    ...
    type: str = Field(default="SemanticStatementOfWork", alias="@type", const=True)
